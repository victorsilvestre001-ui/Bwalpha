...
async function postSignalToTelegram(signal) {
    ...
    const text =
        `${emoji} *NOVO SINAL* — ${signal.pair}\n\n` +
        `Direção: *${signal.direction === 'BUY' ? 'COMPRA' : 'VENDA'}*\n` +
        `Entrada: \`${signal.entry_price ?? '-'}\`\n` +
        `Stop: \`${signal.stop_loss ?? '-'}\`\n` +
        `Alvo: \`${signal.take_profit ?? '-'}\`\n\n` +
...
async function setupWebhook() {
    if (!process.env.TELEGRAM_BOT_TOKEN || !process.env.RAILWAY_PUBLIC_DOMAIN) return;
    ...
    try {
        ...
        if (data.ok) {
            ...
            console.error('Erro ao configurar webhook do Telegram:', data.description);
        }
    } catch (err) {
