...
const { authMiddleware, requirePaidPlan } = require('../middleware/auth');

const router = express.Router();

// Webhook: recebe sinais do TradingView (BWAlpha) ou de outro algoritmo
// Configurar no TradingView um alerta apontando para: POST /api/signals/webhook
// Proteger com uma chave secreta simples via query string ou header
...
// Listar sinais (área de membros — só assinantes veem sinais recentes)
...
// Estatísticas agregadas (win rate, total de sinais, etc.)
...
// Atualizar resultado de um sinal (fechar como win/loss)
router.patch('/:id/close', authMiddleware, async (req, res) => {
    const { id } = req.params;
    const { status, result_pips } = req.body; // status: win | loss | breakeven
