...
export function standardSchemaToJSONSchema(schema, jsonSchema) {
    ...
    throw new AnthropicError(`Could not derive a JSON Schema from this ${std.vendor} schema as it does not implement Standard JSON Schema (\`~standard.jsonSchema\`). Pass the \`jsonSchema\` option, or use a library version that implements it (e.g. zod >= 4.2, or valibot via \`toStandardJsonSchema()\`).`);
}
/**
...
export function parseWithStandardSchema(schema, value) {
    ...
    if (result instanceof Promise) {
        ...
        result.catch(() => { });
        throw new AnthropicError(`Async validation is not supported: the ${schema['~standard'].vendor} schema's \`~standard.validate()\` returned a Promise.`);
    }
    if (result.issues) {
        ...
        throw new AnthropicError(`Schema validation failed with ${issueCount} issue(s):\n${formattedIssues}${suffix}`);
    }
    return result.value;
}
